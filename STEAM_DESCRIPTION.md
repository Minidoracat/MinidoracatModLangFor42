...
[h2]✨ 功能特色[/h2]
[list]
[*] 680+ 個 Workshop 模組（850+ 個模組 ID）的中文翻譯
[*] 有效覆蓋率約 100.0%（截至 2026-09-17）——現行有效上游翻譯鍵 117,255 個中已覆蓋 117,242 個；餘 6 鍵來自已下架、無法重新下載的 FR Operator，另 7 筆為上游檔頭註記、沒有對應物品的錯名鍵或依引擎路由改放其他檔案的按鍵名稱，不補造失效翻譯。上游每天更新，新增文本由追蹤器偵測後持續補譯，故此數字會隨上游浮動。另有少數模組把文字寫死在程式碼裡，任何翻譯包都補不了（已於支援清單逐一標註）
