...
- 小地圖／世界地圖的原版選項名稱（建築、詳細標記、等軸測投影、其他玩家標記等 6 項）改由本體中文翻譯包提供，本包不再出貨；同時安裝多個中文翻譯時，不會再因載入順序出現不同名稱，也修正了「等距縮放」的誤譯。
  > 技術要點：6 個 `IGUI_MapOption_*` 鍵登記進 `sources/unshipped_keys.json` 新類別（原版選項鍵，由本體翻譯包負責），真相層保留；CatLangFor42 1.26.2 已補齊對應譯名，MiniMap 0.28.4 fallback 用詞一致。
...
- **跟上 7 個原已支援模組，補譯 233 個新字串、修訂 22 個既有說明。** 繁中與簡中皆逐鍵對照新版原文維護。
- **AllInfo 補上 183 個字串並修訂 22 個說明。** 新版設定分類、釣魚、畜牧、醫療與裝備提示補齊中文；負重、耐力、恐慌及駕駛特性說明跟上新版。
- **Tikitown 補上 31 個字串。** 急救練習假人、六種練習配方，以及 Proxima 醫療訓練錄影帶與字幕都有中文。
- **Extraction Mode 補上 7 個字串。** 合作與撤離 PvP 模式、突襲輪換間隔及附近撤離訊號都有中文，並清楚說明 PvP 設定不影響合作模式。
- **Cats Mod 補上 6 個字串。** 貓咪突然撲來的提示、趴上肩膀快捷鍵及替代按鍵說明補齊中文。
- **Smart Radial Menu 補上 4 個新版設定字串。** 自訂模組選單與原版環形選單的顯示順序都有中文。
- **Take A Bath And Shower 補上 2 個設定字串。** 自動更換衣物時保留哪些裝備，以及沐浴仍會弄溼衣物的限制，現在都有完整說明。
- **支援版本與安裝方式不變。** 仍支援 Build 42.20.4+；重新啟動遊戲即可載入新翻譯，無須重開存檔。

### Added

- 新增 233 個雙語字串：AllInfo 183、Tikitown 31、Extraction Mode 7、Cats Mod 6、Smart Radial Menu 4、Take A Bath And Shower 2。

### Changed

- 修訂 AllInfo 的 22 個特性說明，跟進新版數值、條件與效果。
- 移除 AllInfo、Dead Magic 與 Tikitown 已不再使用的 61 個原創舊字串；現行附魔物品與石灰漿名稱保留既有中文。

### Fixed

- AllInfo 不再沿用搜刮屍體增加壓力、短眠者額外恢復及舊版駕駛速度限制等過時說明。
- 補齊自動換裝保留項目的大小寫、分隔方式與沐浴效果限制。

### Notes

- 本次只維護原已支援模組的文本；未同步 As1 快照，未新增或維護 Lua 相容層。
- 遊戲本體翻譯保護、最低支援版本、Mod ID 與 Workshop ID 維持不變。

## [42.20.4-1.27.5] - 2026-09-12

### 玩家摘要

- **跟上 6 個原已支援模組，補譯 357 個新字串、修訂 65 個既有字串。** 繁中與簡中皆逐鍵對照新版原文維護。
- **Tomb's Wardrobe 替代版補上 129 個字串。** 新版服飾穿戴選單、背包容量與減重設定補齊中文；墨鏡的高低位置不再誤用低腰穿著的說法。
- **Extraction Mode 補上 97 個字串並修訂 11 個說明。** 新交易品項、服務與車輛路線封鎖提示都有中文；死亡處理補上保留全部裝備選項，並釐清隨機損失裝備的向上取整規則。
- **Global Storage SiK 補上 39 個字串並修訂 28 個說明。** 存取確認、目錄傳輸、區域規則與磁碟操作補齊中文；清楚區分終端機已安裝軟碟機與物品欄持有裝置的條件。
- **Dynamic Traits 補上 56 個字串並修訂 12 個名稱與說明。** 新特質、情緒管理書製作與設定補齊中文；過敏、感冒與選配能力懲罰說明跟上新版，木工不再誤稱伐木工人。
- **Cats Mod 補上 18 個字串。** 貓咪外出提袋、收納與放出動作，以及距離、忙碌與乘車限制都有中文，貓碗名稱也能在新版正確顯示。
- **Simple Suppressors 補上 18 個字串並修訂 14 個說明。** 機油存量、乾涸損傷、高溫磨損、消音範圍與製作要求跟上新版，不再顯示舊的耐久發數與漏油規則。
- **支援版本與安裝方式不變。** 仍支援 Build 42.20.4+；重新啟動遊戲即可載入新翻譯，無須重開存檔。

### Added

- 新增 357 個雙語字串：Tomb's Wardrobe 替代版 129、Extraction Mode 97、Dynamic Traits 56、Global Storage SiK 39、Cats Mod 18、Simple Suppressors 18。

### Changed

- 修訂 65 個既有字串：Global Storage SiK 28、Simple Suppressors 14、Dynamic Traits 12、Extraction Mode 11。
- 跟進服飾與特質模組的新版命名，保留其他模組共用文字的中性譯名。
- 移除 Cats Mod 與 Simple Suppressors 已不再使用的 6 個舊字串。

### Fixed

- 修正木工被誤稱為伐木工人，以及新版墨鏡選單誤用低腰穿著的問題。
- 補回 Global Storage SiK 對軟碟機必須安裝於終端機的條件。
- 補齊 Extraction Mode 簡中死亡處理的適用範圍與隨機損失裝備的取整說明。
...
## [42.20.4-1.27.4] - 2026-09-11

### 玩家摘要

- **跟上 9 個原已支援模組，補譯 294 個新字串、修訂 177 個既有字串。** 繁中與簡中皆逐鍵對照新版原文維護。
- **Pixel Strike Indicator 補上 192 個字串並修訂 166 個設定與說明。** 世界資訊規則、血條與傷害數字的預設外觀、設定代碼分享、存檔槽與預覽操作補齊中文；權限限制、連續命中合併及減少動態的說明跟上新版。
- **MedievalZ 補上 51 個裝備與製作文字。** 新增的中世紀護甲、零件與不同樣式罩袍都有中文，物品名稱與製作選單保持一致。
- **M923 軍用卡車補齊 16 個新版製作選單。** 車門、裝甲、貨箱木板等配方不再因新版更換名稱而顯示英文。
- **自動操作與貓咪互動補齊中文。** AutoAll 新增 14 個、修訂 5 個字串，彈藥裝盒與裝箱的來源及剩餘數量更清楚；Cats Mod 補上 8 個呼喚、距離與冷卻提示；Sleep-to-Sleep 補上 7 個休息統計文字。
- **PhunCure 的機率單位不再混淆。** 兩個版本分別使用萬分比與百分比，現在會並列各自的填值方式；PhunCure 2 另補上 6 個診斷提示。
- **統計與停水停電說明更準確。** KillCount 釐清死亡後統計與不重置伺服器的設定；Immersive Blackouts 說明日期欄位變空白代表輸入格式無效。
- **支援版本與安裝方式不變。** 仍支援 Build 42.20.4+；重新啟動遊戲即可載入新翻譯，無須重開存檔。

### Added

- 新增 294 個雙語字串：Pixel Strike Indicator 192、MedievalZ 51、M923 軍用卡車 16、AutoAll 14、Cats Mod 8、Sleep-to-Sleep 7、PhunCure 2 診斷提示 6。

### Changed

- 修訂 177 個既有字串：Pixel Strike Indicator 166、AutoAll 5、KillCount 2、Immersive Blackouts 2、PhunCure 共用機率說明 2。
- Pixel Strike Indicator 的世界權限、個人外觀、設定代碼與預覽操作說明跟進新版，移除一個上游已退役的舊分類文字。

### Fixed

- PhunCure 兩版本的掉落率說明分別標明萬分比與百分比，避免以錯誤單位填寫設定。
- 修正 KillCount 對不重置伺服器的設定指示，以及 Pixel Strike Indicator 對未受光照目標的說明。
- 修正新版 M923 製作選單缺少中文的問題。
...
## [42.20.4-1.27.3] - 2026-09-10

### 玩家摘要

- **補譯 98 個新字串，修訂 38 個既有字串。** 繁中與簡中皆逐鍵對照各模組新版原文維護。
- **Cats Mod 補上 67 個字串。** 貓咪迷路與重逢、送禮、天氣反應、個性和相伴統計都有中文。
- **Simple Suppressors 補上 26 個字串並修訂 36 個設定與說明。** 消音器相容性、熱度顯示、製作進程及口徑選擇補齊中文；機油說明跟上新版，不再誤寫成定期換油或拆下濾清器就立刻損傷引擎。
- **Extraction Mode 的任務地點跟上新版。** 零號病人樣本改至路易斯維爾總醫院尋找；另補上車庫缺少提示與供暖目標溫度設定，共新增 3 個、修訂 2 個字串。
- **Better Clothing Info 補上 2 個設定文字。** 衣物除錯資訊開關與說明補齊中文，可辨識物品類型及穿戴部位。
- **Motorious Zone 新增車系保留各自原名。** 虛構品牌與真實車名附加模組共用同一個代號，維持各自的英文名稱，避免中文翻譯改掉玩家選用的命名方式。
- **支援版本與安裝方式不變。** 仍支援 Build 42.20.4+；重新啟動遊戲即可載入新翻譯，無須重開存檔。

### Added

- 新增 98 個雙語字串：Cats Mod 67、Simple Suppressors 26、Extraction Mode 3、Better Clothing Info 2。

### Changed

- Simple Suppressors 的 36 個設定名稱與說明跟進新版分類、口徑開關、機油損傷門檻及舊換油設定的停用狀態。
- Motorious Zone 新增車系沿用既有命名衝突裁決，保留主模組與真實車名附加模組各自的英文名稱。

### Fixed

- 修正 Extraction Mode 兩段零號病人樣本任務文字仍指向舊醫院的問題。
- 修正 Simple Suppressors 機油過低損傷的間隔、加倍條件與預設值說明。
...
## [42.20.4-1.27.2] - 2026-09-09

### 玩家摘要

- **跟上 11 個原已支援 MOD 的文本更新，補譯 536 個新字串、修訂 118 個既有字串。** 繁中與簡中皆逐鍵對照原文維護。
- **SnackTime89 補上 376 個字串。** 未開封麥片、收藏卡與貼紙名稱、卡片說明及檢視選單補齊中文，年份與收藏編號完整保留。
- **Simple Suppressors 補上 37 個字串並修訂 88 個說明。** 機油補充與保養、漏油、消音器維修及受熱效果都有中文；製作需求、耐久度與戰利品設定跟上新版。
- **Knox Chronicles 補上 37 個字串並修訂 18 段任務文字。** 供水任務的兩階段維修、材料與工具需求、危險提醒及獎勵更清楚，並補上主選單閃光效果的光敏警語。
- **自動化與特質設定跟上新版。** AutoAll 的操作與設定文字、Evolving Traits World 的哮喘及勇氣進度說明，繁簡兩側同步更新。
- **其他模組補譯。** P4 My-So-Called Snack 新增 12 張小浣熊男孩貼紙及說明；另補上 WayMoreCars、ExerciseWithCorpses、Functional Appliances 與 PSR 的新增或改寫文字。
- **服飾選單更精確。** Frockin Splendor 補上 12 個分類文字，並修正共用的衣物閉合與往下調整動作，避免把其他模組的操作誤寫成扣夾克或穿低衣物；左右穿戴方向尚待上游釐清，維持原值。
- **支援版本與安裝方式不變。** 仍支援 Build 42.20.4+，Mod ID 與 Workshop ID 不變；重新啟動遊戲即可載入新翻譯，無須重開存檔。

### Added

- 新增 536 個雙語字串：SnackTime89 376、Simple Suppressors 37、Knox Chronicles 37、AutoAll 24、P4 My-So-Called Snack 24、Evolving Traits World 16、Frockin Splendor 12，以及其他原已支援模組合計 10 個。

### Changed

- 修訂 118 個既有字串，繁中與簡中皆跟進新版原文；不以自動簡繁轉換取代翻譯。
- 收藏品沿用現行食品品牌與貼紙系列譯名，保留卡片上的專名、年份及編號。

### Fixed

- 修正消音器戰利品、磨損、音量與相容性說明中的過時條件，並避免把單次補油量誤寫成最終油位上限。
- 修正共用服飾選單過度限定衣類與動作的譯文；未能證實的左右方向不擅自改寫。

### Notes

- 本次只維護原已支援模組的文本；未同步 As1 快照，也未新增或維護 Lua 相容層。
- Frockin Splendor 的左右穿戴文字仍待上游釐清，相關問題保留追蹤；不將尚未證實的方向標示為已完成。
- 遊戲本體翻譯保護與最低支援版本維持不變。

## [42.20.4-1.27.1] - 2026-09-08

### 玩家摘要

- **跟上 14 個 MOD 的上游更新，補譯 207 個新字串、修訂 133 個既有字串。** 繁中與簡中皆逐鍵對照原文維護。
- **Hot Brass Ammo Crafting 補上 85 個字串。** 彈頭、底火、整盒材料、裝填工作檯，以及組裝、拆解與裝盒配方都有中文。
- **Burd's Survival Journals 補上 53 個新字串並修訂 11 個說明。** 涵蓋日記維護、收益遞減恢復、每位玩家獨立領取與 Lifestyle 志向還原；材料名稱與遊戲物品欄一致。
- **Global Storage SiK 補上 39 個新字串並修訂 69 個說明。** 分類修正、無主網路繼承、管理員選項與物品轉移狀態皆跟上新版；容器範圍與終端連結距離的說明分開呈現。
- **Cats Mod 的自動活動設定補齊中文。** 睡意、玩耍、抓癢、趴肩與伸展等 16 個狀態或設定文字，保留各自的距離、次數與冷卻條件。
- **食品與電子物品名稱跟上原文。** P4 My So-Called Snack 的巧克力、貼紙改用 SPIFFOMAN 新名稱；Better Electronics 的簡中警報器與手冊名稱不再沿用過時標題，並補上水獺鼻。
- **修正容易誤解的說明。** AmmoMaker 的產量單位由舊文 0.02g 更新為 20g；Simple Suppressors 明確說明 .44 消音器可裝在沙漠之鷹，但不支援 .44 馬格南左輪手槍；兩則貼紙名稱移除多餘反斜線。
- **其他更新。** Authentic Z 水兵帽、TNFP 彈匣、OC's Packing 木炭整箱、AutoLoot 排除不需要物品、Guns of Marz 彈匣分類，以及 Arcadia 小型丙烷儲存站設定文字。
- **安裝方式與支援版本不變。** 仍支援 Build 42.20.4+，Mod ID 與 Workshop ID 不變；重新啟動遊戲即可載入新翻譯，無須重開存檔。

### Added

- 新增 207 個雙語字串：Hot Brass Ammo Crafting 85、Burd's Survival Journals 53、Global Storage SiK 39、Cats Mod 16，以及其他原已支援模組合計 14 個。

### Changed

- 依原文更新 133 個既有字串；其中繁中實際改值 122 個、簡中實際改值 101 個，兩語分別審核，不以自動簡繁轉換代替翻譯。
- 移除日記與全域倉儲模組已不再使用的 10 個原創舊字串；已確認沒有其他模組繼續使用這些代號。

### Fixed

- 修正貼紙名稱中的多餘反斜線，並將日記維護所需材料對齊遊戲官方名稱。
- 修正 AmmoMaker 與 .44 消音器的過時說明，以及 Better Electronics 簡中名稱與原文不符的問題。

### Notes

- 本次只維護原已支援模組的文本更新；未新增 MOD、未同步 As1 上游快照，也未新增或維護 Lua 相容層。
- 保留原有遊戲本體翻譯保護；最低支援版本仍為 Build 42.20.4。
...
- **同步 As1 v3.8.0，新增 23,715 個翻譯鍵並更新 38 筆既有譯文。** 本次新增內容涵蓋
  物品、槍械、彈藥、配方、特質、介面與沙盒設定；所有繁中／簡中均依現行上游文字逐項審核。
- **新增或補齊 86 個 Workshop 項目的追蹤與支援資訊。** 支援清單目前包含 658 個在架
  Workshop MOD、834 個 mod ID，另保留 16 個已下架項目的翻譯。
- **清償 #319–#327 全部 tracker issue。** Authentic Z、Herbalist、ETW、RealFirearms、
  Organized Categories、GaelGunStore、ModernFirearmsSystem 等更新已跟進，現行缺口為 0。
- **強化翻譯正確性與安全性。** 修正口徑、發數、型號與舊配方名稱；不同 MOD 共用同一
  代號時採中性譯名或停止出貨衝突鍵，且持續維持對遊戲本體 48,718 個翻譯鍵零覆寫。
- **追蹤管線改為第一手證據歸屬。** 新 MOD 可先登記追蹤，再由上游 EN 鏡像建立 owner；
  stale watchlist、部分鏡像遺失、壞 worklist 與不完整來源樹都會在出貨前阻斷。
