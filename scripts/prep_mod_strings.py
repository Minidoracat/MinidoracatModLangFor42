...
def render_owner_report(decided: dict, census: dict[str, dict[str, str]],
    ...
    **為什麼放在本檔而不是 `build_mod.py`**（#245 項目 2 原本建議比照 `manifest`）：
    ...
    out = [
        ...
        "**顯示代號而非英文**。要解決得請該 MOD 作者改用 `.json`。 |",
        "",
        "**上游回報責任**：本包負責把可驗證的檔名、鍵名、上游原文與機制原因查清楚並"
        "記錄在本檔，但**不代為向 MOD 作者回報，也不代為追蹤上游是否修復**。"
        "需要該 MOD 中文化的玩家（實際使用者或翻譯申請者）請自行拿本檔資訊向原作者反映。"
        "上游把文字改成標準的 JSON 翻譯機制、或把撞名的代號改開之後，本包就能接手翻譯；"
        "在那之前維持現行裁決。「已回報上游」欄僅在有人回報並提供連結時才會填寫。",
        "",
