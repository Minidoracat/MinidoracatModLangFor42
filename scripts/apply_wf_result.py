...
# terminology 規則去逐鍵裁決即可。
# **帶 guard 的三個詞（默認／內存／視頻）的 pattern 必須與 `sources/terminology.json`
# 的 `match.value` 逐字一致**：那三條規則的 guard 是本體語料實測出來的，少了它就會把
# 台灣正當語境判成陸用語而拒掉整批——「藏身點內存活」「屋內存放物資」「沉默認同」
# 「檢視頻率」都是合法 CH，語料已有多筆（實例：IGUI_ExtractionMode_Error_QuestHideoutOnly
# 的「你必須在藏身點內存活」）。「電視頻道」由 `(?<![檢電監注收凝巡])` 的「電」擋掉。
MAINLAND = (
    "信息", "軟件", "屏幕", "網絡", "鼠標", "硬盤", "服務器",
    r"(?<![沉幽])默認(?![了著])",
    r"內存(?![在放有著活於糧貨款量檔])",
    r"(?<![檢電監注收凝巡])視頻",
)
MAINLAND_RE = re.compile("|".join(MAINLAND))
